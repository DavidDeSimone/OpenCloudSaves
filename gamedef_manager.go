...
func (d *GameDef) GetWindowsInstallLocationFromSteam() string {
	...
	libraryFoldersStr, err := os.Open(path.Join(steamPath, "steamapps", "libraryfolders.vdf"))
	if err != nil {
		log.Panic(err)
	}
	...
	for _, libraryFolder := range libraryFolders.LibraryFolders {
		for steamId := range libraryFolder.Apps {
			if steamId == d.SteamId {
				steamAppsDir := path.Join(libraryFolder.Path, "steamapps")

				appManifestStr, err := os.Open(path.Join(steamAppsDir, fmt.Sprintf("appmanifest_%s.acf", d.SteamId)))
				if err != nil {
					log.Panic(err)
				}
				...
				result := path.Join(steamAppsDir, "common", appManifest.AppState.InstallDir)
				return result
			}
		}
	}
...
func (d *GameDef) GetSyncpaths() ([]Datapath, error) {
	...
	// d.GetWindowsInstallLocationFromSteam()
